...
class Channel(virtual.Channel):
    ...
    def _connparams(self, a_sync=False, _r210_options=(
    ...
    def _create_client(self, a_sync=False):
        if a_sync:
            return self.AsyncClient(connection_pool=self.async_pool)
        return self.Client(connection_pool=self.pool)

    def _get_pool(self, a_sync=False):
        params = self._connparams(a_sync=a_sync)
    ...
    def async_pool(self):
        if self._async_pool is None:
            self._async_pool = self._get_pool(a_sync=True)
        return self._async_pool

    @cached_property
    def client(self):
        """Client used to publish messages, BRPOP etc."""
        return self._create_client(a_sync=True)

    @cached_property
    def subclient(self):
        """Pub/Sub connection used to consume fanout queues."""
        client = self._create_client(a_sync=True)
